...
from analytic_tables.conversion_functions import get_beginning_and_end_of_month, validate_and_convert_state, \
    add_income_level, add_foster_logic
...
def transform_enrollment_df(enrollment_df: pd.DataFrame, month_start: datetime.date, month_end: datetime.date) -> pd.DataFrame:
    ...
    enrollment_df['Gender'] = enrollment_df['Gender'].replace({'M ': MonthlyEnrollmentReporting.MALE,
...
def get_region(town_name: str) -> str:
    ...
    return town_lookup.get(town_name.lower(), None)
...
#
# def add_time_and_age_group(df: pd.DataFrame) -> pd.DataFrame:
#     """
#     Add
#     :param df: string from ECIS
#     :return:
#     """
#     df[MonthlyEnrollmentReporting.CDCTimeName.name] = np.where(df[MonthlyEnrollmentReporting.FundingSource.name],
#                                                                'test',
#                                                                None,)
#     df[MonthlyEnrollmentReporting.SpaceType.name] = np.where(df['SpaceType']
#
#
