...
class TestSQLExtraction(unittest.TestCase):
	...
	def test_space_capacity(self):
		...
		lookup_dict = df.set_index(['TimeName', 'AgeGroupName']).to_dict()['Capacity']
	...
	def test_space_utilization(self):
		...
		df = pd.read_sql(sql=query, con=self.conn)
		lookup_dict = df.set_index(['TimeName', 'AgeGroupName']).to_dict()
		...
		total_dict = lookup_dict['UtilizedTitleISpaces']
		self.assertEqual(total_dict[(FT, INFANT)], 1)

		# Check Standard utilized numbers
		total_dict = lookup_dict['UtilizedNonTitle1Spaces']
		self.assertEqual(total_dict[(FT, INFANT)], 4)
		self.assertEqual(total_dict[(PT, INFANT)], 2)
		self.assertEqual(total_dict[(FT, PRESCHOOL)], 1)
		self.assertEqual(total_dict[(PT, PRESCHOOL)], 1)
		self.assertEqual(total_dict[(FT, SCHOOL_AGE)], 1)
